...
from src.rl.posture_dataset import UTILITY_MODES, build_posture_dataset, save_posture_dataset
...
@click.option(
    "--utility-mode",
    type=click.Choice(list(UTILITY_MODES), case_sensitive=False),
    default="full_utility",
    show_default=True,
    help="Primary utility mode used for winner/margin summaries.",
)
@click.option(
    "--near-tie-threshold",
    default=0.01,
    type=float,
    show_default=True,
    help="Utility margin below which a label is treated as a near tie in summaries.",
)
@click.option("--config", default=None, help="Path to custom config file")
def main(start_date, end_date, horizon_rebalances, max_samples, prefix, utility_mode, near_tie_threshold, config):
    ...
    saved = save_posture_dataset(
        ...
        prefix=prefix or f"posture_dataset_{utility_mode.lower()}_h{int(horizon_rebalances)}",
    )
